...
public class Map : MonoBehaviour
{
	...
	public Button buttonEnableMagic;
	...
	public void ChangeUnit (GameObject Cu)
	{
		...
		//If the selected player is on the first player team or on second player team and it is their turn
		if (firstPlayerTurn && selectedUnit.tag.Equals ("Player 1") && selectedPlayer.magicAttack > 0) {
			buttonEnableMagic.interactable = true; //Working
		} else {
			buttonEnableMagic.interactable = false;

			if (!firstPlayerTurn && selectedUnit.tag.Equals ("Player 2") && selectedPlayer.magicAttack > 0) {
				buttonEnableMagic.interactable = true; //Working
			} else {
				buttonEnableMagic.interactable = false;
			}
	...
	{//change so one doesnt spawn on other objects.
		if ((selectedPlayer.selected && firstPlayerTurn && selectedPlayer.tag.Equals ("Player 1")) || (selectedPlayer.selected && !firstPlayerTurn && selectedPlayer.tag.Equals ("Player 2"))) {
			for (int x = 0; x < sizeX; x++) {
				for (int y = 0; y < sizeY; y++) {
	...
	public void moraleChange (ClickUnit cu)
	{
		...
		player2MoraleText.text = "Player 2 Morale: " + player2Morale;
	}
		...
		if (!firstPlayerTurn) {
			for (int i = 0; i < player1Units.Length; i++) {
				if (player1Units [i] != null && player1Units [i].transform.position.x == x && player1Units [i].transform.position.y == y && selectedPlayer.movesLeft != -1) {
					...
					if (selectedPlayer.magicEnabled) {
						attackedPlayer.health -= selectedPlayer.magicAttack;
						selectedPlayer.magicEnabled = false;
					} else {
